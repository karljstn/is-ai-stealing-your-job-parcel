...
class HandOK extends withMouse implements ThreeGLTF {
	...
	initialize = () => {
		setTimeout(() => {
			...
			const clip = AnimationClip.findByName(this.animations, "ArmatureAction.002");
			this.okayAction = this.mixer.clipAction(clip)
			this.okayAction.loop = LoopOnce
			this.okayAction.clampWhenFinished = true

			this.setTransition(MODELS.HAND_OK.SCALE, this.group.position)
	...
	tweaks = () => {
		...
		const sizeInput = folder.addInput(this.params, 'size', { label: "Hand size", min: MODELS.HAND_OK.SCALE * 0.33, max: MODELS.HAND_OK.SCALE * 3 })
