...
class Hand {
	...
	constructor(size: number, pane: Tweakpane | null) { //camera: PerspectiveCamera, renderer: WebGLRenderer
		...
		this.loader.load(MODELS.HAND.url, (gltf) => {
			...
			// Tweakpane
			const speedInput = pane?.addInput(this.params, 'animSpeed', { label: "Wave speed", min: this.params.animSpeed * 0.33, max: this.params.animSpeed * 3 })
			const sizeInput = pane?.addInput(this.params, 'size', { label: "Hand size", min: size * MODELS.HAND.scale * 0.33, max: size * MODELS.HAND.scale * 3 })
			const btn = pane && pane.addButton({ title: "Wave" })
