 ...
 class HandWave extends MouseGLTF implements ThreeGLTF {
 	...
 	mat: MeshLambertMateria
 	constructor(scene: Scene, viewport: Viewport) {
 		super(scene, viewport)
 		this.params = {
 			animation: { speed: 0.002 },
 			size: MODELS.HAND_WAVE.BASE_SCALE
 		}
 	...
 	initialize = () => this.setFromRect(RECTS.INTRO.HELLO).then(({ x, y, w, h }) => {
 		...
 		this.mixer.timeScale = this.params.animation.spee
 		...
 		this.waveAction.clampWhenFinished = tru
 		this.setTransition(MODELS.HAND_WAVE.BASE_SCALE, this.group.position
 	...
 	tweaks = () => {
 		if (!store.state.tweakpane) retur
 		const folder = store.state.tweakpane.addFolder({ title: 'Hand', expanded: false }
 		const speedInput = folder.addInput(this.params.animation, 'speed', { label: "Wave speed", min: this.params.imation.speed * 0.33, max: this.params.animation.speed * 3 })
 		const sizeInput = folder.addInput(this.params, 'size', { label: "Hand size", min: MODELS.HAND_WAVE.BASE_SCALE * 0., max: MODELS.HAND_WAVE.BASE_SCALE * 3 })
