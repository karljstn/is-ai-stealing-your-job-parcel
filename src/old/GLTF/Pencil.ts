...
class Pencil extends TweenGLTF implements ThreeGLTF {
	...
	initialize = () => {
		...
		this.setTransition(MODELS.PENCIL.BASE_SCALE, this.group.position, new Vector3(0, 0, 0),)
	...
	tweaks = () => {
		...
		const sizeInput = folder?.addInput(this.params, 'size', { label: "Hand size", min: MODELS.PENCIL.BASE_SCALE * 0.2, max: MODELS.PENCIL.BASE_SCALE * 2 })
