<template>
	<section>
		<div ref="gameContainer" class="game-container" :class="this.getClassForContainer">
			<Side></Side>
			<ButtonsRight
...
export default Vue.extend({
	...
	mounted() {
		...
		// console.log(store.state.sceneManager?.scene);

		// store.state.sceneManager?.renderer.setClearColor(0x231f38, 1);
		...
		if (!store.state.devMode.forceRadiologist) {
			...
			// store.state.sceneManager?.Loader?.fullScreenPlane.hide();
		}
	},
	destroyed() {
		// store.state.sceneManager?.renderer.setClearColor(0x000000, 1);
		if (!store.state.devMode.forceRadiologist) {
