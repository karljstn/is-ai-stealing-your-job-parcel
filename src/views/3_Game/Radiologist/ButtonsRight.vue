<template>
	<div class="button-container">
		<div class="ai-button-container" v-on:click="this.useAI" :class="this.classAIBtn">
			<button class="ai-button" v-bind:class="{ tutorialOpened: timerPause || !timerCanStart }"></button>
	...
	methods: {
		useAI() {
			if (this.timerCanStart && !this.timerPause && !this.usedAI) {
				store.state.sceneManager?.radio.useAI();
				this.usedAI = true;
			}
		},
		...
		openPatientFile() {
			if (this.timerCanStart && !this.timerPause) {
				...
				store.state.sceneManager?.radio.patientFile(this.patientFile);
			}
		},
	},
	watch: {
